 ...
 */
static inline int multi_test_and_set_bit(uint16_t nr, uint8_t *addr)
{
 ...
 * return:  
 */
static inline void multi_clear_bit(uint16_t nr, uint8_t  *addr)
{
    ((uint8_t *)addr)[nr / BITS_PER_BYTE] &= ~(1U << (nr % BITS_PER_BYTE));
}
 ...
 * return:  
 */
int put_idx(uint8_t *base_addr, uint32_t idx)
{
 ...
 */
int get_idx(uint8_t *base_addr, uint32_t *idx)
{
    uint32_t i;
...
int multi_node_init(void)
{
    ...
    memset(g_bitmap , 0 , sizeof(uint8_t) * BITMAP_SIZE);  
    return 0;
}
